    ...
    function renderResult(outcome, confidence, ingredients) {
        ...
        if (btnDownload) {
            ...
            newBtn.addEventListener('click', () => {
                ...
                let reportText = `KIBBLESCAN TOXICOLOGY AUDIT\n`;
                reportText += `Date: ${timestamp}\n`;
                reportText += `Status: ${outcome}\n`;
                reportText += `Confidence: ${(confidence * 100).toFixed(0)}%\n\n`;

                reportText += `INGREDIENT ANALYSIS:\n`;

                if (ingredients && ingredients.length > 0) {
                    ingredients.forEach(ing => {
                        if (ing.flagged) {
                            reportText += `[!] ${ing.name}\n`;
                            reportText += `    Risk: ${ing.reason || 'Restricted Agent'}\n`;
                            reportText += `    Cit: Global Toxicology Registry v2.4\n\n`;
                        } else {
                            reportText += `[✓] ${ing.name}\n`;
                        }
                    });
                } else {
                    reportText += `(No legible ingredients detected)\n`;
                }

                reportText += `\n--\n`;
                reportText += `Generated by KibbleScan (kibblescan.com)\n`;
                reportText += `Independent Veterinary Standard.`;
